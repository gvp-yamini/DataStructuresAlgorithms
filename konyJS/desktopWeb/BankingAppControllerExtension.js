...
kony.sdk.mvvm.BankingAppControllerExtension = Class(kony.sdk.mvvm.BaseFormControllerExtension, {
   ...
   fetchData: function(successCallback, errorCallback) {
         ...
         if (this.fetchMasterData && (typeof this.fetchMasterData === "function")) {
                this.fetchMasterData(serviceName, options, onSuccess, customErrorCallback);
         }else{
	       		onSuccess(null);
	     }
	      function onSuccess(response) {	        
	      	kony.sdk.mvvm.BankingAppControllerExtension.$superp.fetchData.call(scopeObj,successCallback, customErrorCallback);
	      }
    ...
    saveRecords: function(recordsArray,successcallback, errorcallback) {
       kony.sdk.mvvm.BankingAppControllerExtension.$superp.saveRecords.call(this,recordsArray,successcallback, customErrorCallback);
    },
    saveRecord: function(record,successcallback, errorcallback) {
       kony.sdk.mvvm.BankingAppControllerExtension.$superp.saveRecord.call(this,record,successcallback, customErrorCallback);
    },
    deleteData: function(success, error) {
       kony.sdk.mvvm.BankingAppControllerExtension.$superp.deleteData.call(this,success, customErrorCallback);
    },
  ...
  fetchMasterData: function(serviceName, options, successcallback, errorcallback) {
    ...
    getPickListValues : function(pickListInfo,widgetId,successcallback, errorcallback){
        ...
        var frmName = this.getController().getConfig().getFormId();
		var entity = this.getController().getConfig().getEntity();
		eval(" var frmConfigClassObj = " +frmName+"Config");
        ...
        var options = {"dataObject":dataObject, "headers":headers};
 	    objectService.fetch(options, success,customErrorCallback);
        ...
        function success(response){
			if(response){
                ...
                var masterDataElement = [];
                 if(frmConfigClassObj[widgetId].fieldprops.store !== undefined)
        {
          switch(entity)
          {
            case "Accounts": 
              kony.retailBanking.globalData.accounts.setAccountsData(response.records);
              break;   
          }
        }else if(frmConfigClassObj[widgetId].fieldprops.selector === undefined)
        {
...
if("frmMakeTransferKA" === frmName || "frmPayAPersonKA" === frmName){    
          masterData=fetchDataForTransferAccounts(response, pickListInfo,widgetId,scopeObj);
          kony.print("MasterData received from functionCall "+JSON.stringify(masterData));
        }
        else{     
        for (var i = 0; i < response.records.length; i++) 
        {
          ...
          if(value === "" && entity === "Accounts")
          {
            value = response.records[i][pickListInfo.alter] + response.records[i][pickListInfo.key].slice(-4); 
          }else if(value === "")
          {
            value = response.records[i][pickListInfo.alter];   
          }
