...
kony.retailBanking.util.APPLICATION_DATE_TIME_FORMAT = "m/d/Y h:i A";
...
kony.retailBanking.util.formatingAmount.appendingCurrencyCodeToAmount = function(amount, currencyCode) {
    ...
    var formatLocale = kony.retailBanking.util.formatingAmount.getLocaleFormatString();
    if (currencies[formatLocale] == undefined){
       if(amount.indexOf(kony.retailBanking.util.currency_symbols["USD"]) == -1){
      amount = formatAmount(amount);
      return negative + kony.retailBanking.util.currency_symbols["USD"] + "" + amount;
    }else{
      return amount;
    }
    }
	if(amount.indexOf(currencies[formatLocale])== -1){
    amount = formatAmount(amount);
    return negative + currencies[formatLocale] + "" + amount;
	}else{
		return amount;
	}
};
